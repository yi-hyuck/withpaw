...
CREATE OR REPLACE TRIGGER SEQ_TRG_Member
...
COMMENT ON COLUMN Member.user_id IS '회원 고유 식별(자동 생성)';
COMMENT ON COLUMN Member.email IS '회원가입시 사용 이메일';
COMMENT ON COLUMN Member.login_id IS '로그인시 사용할 아이디';
COMMENT ON COLUMN Member.password IS '비밀번호 해시값';

COMMENT ON TABLE Pet IS '반려동물 정보';
COMMENT ON COLUMN Pet.pet_id IS '반려동물 고유 식별(자동 생성)';
COMMENT ON COLUMN Pet.petname IS '반려동물 이름';
COMMENT ON COLUMN Pet.birthdate IS '반려동물 생년월일(yyyy/mm/dd)';
COMMENT ON COLUMN Pet.gender IS '반려동물 성별(F,M)';
COMMENT ON COLUMN Pet.neuter IS '중성화 여부(0,1)';
COMMENT ON COLUMN Pet.weight IS '반려동물 체중(소수점 아래 두자리)';

COMMENT ON TABLE Symptom IS '증상 기록';
COMMENT ON COLUMN Symptom.symptom_id IS '증상 기록 고유 식별(자동 생성)';
COMMENT ON COLUMN Symptom.symptom IS '증상 기록(앱 사용자가 직접 기록)';
COMMENT ON COLUMN Symptom.symptomdate IS '증상 기록 날짜(yyyy/mm/dd)';

COMMENT ON TABLE Food IS '음식 정보';
COMMENT ON COLUMN Food.food_id IS '음식 고유 식별(자동생성)';
...
COMMENT ON COLUMN Breed.breed_id IS '품종 고유 식별(자동생성)';
